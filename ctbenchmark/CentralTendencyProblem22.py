...
class CentralTendencyProblem22(CentralTendencyBenchmarkProblem):
    def __init__(self, mu=[0.5] * 2, sigma=[0.15] * 2):
        ...
        We have x1 ~ Normal(mu[0], sigma[0]) and x2 ~ Normal(mu[1], sigma[1]), both truncated between 0. and 1.
        ...
        X1 = ot.TruncatedDistribution(ot.Normal(mu[0], sigma[0]), 0., 1.)
        X1.setDescription(["X1"])
        X2 = ot.TruncatedDistribution(ot.Normal(mu[1], sigma[1]), 0., 1.)
